...
test.describe("Commandes, payer en une seule fois", () => {
  test("Contexte: aller point de vente 'RESTO'", async ({ browser }) => {
    ...
    console.log('-> 1 - currencySymbolTrans =', currencySymbolTrans)
  })
  /*
    ...
    test('Commande sur table 2, paiement en cashless', async () => {
      ...
      test("Contexte: 0 € sur première carte et 10 Unités pour la deuxième", async () => {
  ...
  */
  // a faire deamin
    test('Commande sur table 5, paiement en cashless, fonds insuffisant sur carte, complémentaire espèce, somme donnée 10', async () => {
      ...
      // bouton "CASHLESS" présent
      await expect(page.locator('#popup-cashless div[class="paiement-bt-container test-ref-cashless"]')).toBeVisible()
  ...
  /*
    test.skip('Commande sur table Ex01, paiement cashless, fonds insuffisant sur carte, complémentaire cb', async () => {
      // cartes vidées sur contexte précédant
    ...
    test.skip('Commande sur table Ex02: total  32.1, paiement en espèce, somme donnée 1er essai 20 et 2ème essai 50', async () => {
      // table éphémère visible
      await expect(page.locator('#tables-liste .test-table-ephemere')).toBeVisible()
    ...
    test.skip('Commande sur table Ex03, paiement en cb', async () => {
      // aller au point de vente RESTO
      await goPointSale(page, 'RESTO')
